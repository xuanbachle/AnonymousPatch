...
public class WalkerState {
  ...
  public void seekToYear(String year) {
    int yearInt = Integer.parseInt(year);System.out.println("Y: "+yearInt);
    //assert(yearInt > 0 && yearInt < 9999);//FIX: assert(yearInt >= 0 && yearInt < 9999);
    if(((yearInt >= 0) && (yearInt < 9999))){
    }else{
    	throw new RuntimeException("axax");
    }
    markDateInvocation();
